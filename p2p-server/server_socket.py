    ...
    def handle_client(self, conn, addr):
                ...
                with registry_lock:
                    for file in payload.get("files",[]):
                        ...
                        if file_name not in file_database:
                            file_database[file_name] = {peer_id:[]}
        ...
        try:   
            if request_type == "FILE_LIST_REQUEST":
                ...
                response = server_response_models.FileListReply(number_of_files=number_of_files,files_list=file_metadata)
        ...
        try:  
            if request_type == "FILE_LOCATIONS_REQUEST":
                ...
                number_of_peers = len(file_database.get(file_name,{}))
                chunkendpoint_map = file_database.get(file_name,{})
                response = server_response_models.FileLocationsReply(number_of_peers=number_of_peers,chunk_endpoint_map=chunkendpoint_map)
                conn.sendall(response.model_dump_json().encode())
        except Exception as e:
            response = {"status":"502","message":str(e)}
            conn.sendall(json.dumps(response).encode())
